...
export interface EscrowState {
  ...
  campaignCapEnabled: boolean // campaign-wide cap on total rewards issued (lifetime)
...
export async function loadEscrowState(escrow: Address): Promise<EscrowState> {
  ...
  const [rateBps, start, end, reward, rewardTokenId, rules, platformFeeBps, platformFeeAccount] = terms as readonly [
    ...
    { minSpendEnabled: boolean; minSpend: bigint; capEnabled: boolean; cap: bigint; dayOfWeekEnabled: boolean; daysOfWeek: number; flatEnabled: boolean; flatValue: bigint; redeemable: boolean; perTxCapEnabled: boolean; perTxCap: bigint; capWindow: number; capWindowCount: number; capWindowTime: number; capWindowDow: number; campaignCapEnabled: boolean; campaignCap: bigint },
    bigint, Address,
  ]
  ...
  return {
    ...
    campaignCapUsd: formatUsd(rules.campaignCap),
